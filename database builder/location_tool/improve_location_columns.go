...
func extractCountryAndState(address, location string) (string, string) {
	...
	// First try to find state in the address
	...
	if len(locationParts) >= 2 {
		...
		// Also try the last part in case it's not "USA"
		lastPart := strings.TrimSpace(locationParts[len(locationParts)-1])
		if lastPart != "usa" {
			if stateAbbr, exists := stateNamesToAbbr[lastPart]; exists {
				return "USA", stateAbbr
			}
		}
	}

	// If still no state found, try to extract from address more carefully
...
func main() {
	if len(os.Args) != 2 {
		fmt.Println("Usage: go run improve_location_columns.go <database_path>")
		os.Exit(1)
	}
